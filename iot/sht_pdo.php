...
if ($_POST["func"] == "average") {
    $date_begin = (isset($_POST["date_begin"])?$_POST["date_begin"]:date('YmdHis'));
    $date_end   = (isset($_POST["date_end"])?$_POST["date_end"]:date('YmdHis'));
    ...
    error_log("day to second ============: $seconds ================");
...
FROM (
      ...
      FROM (
            SELECT CAST(timestamp/(:seconds) as INT)*(:seconds) as span,
               humi, temp
              FROM ta_iot
             WHERE time between :date_begin and :date_end )
       group by span)
...
// 데이터 조회
if ($_POST["func"] == "get_station") {
    $id = $_POST["id"];

    $sql = <<<EOT
SELECT id, name, address, latitude, longitude, url, apikey, graph FROM ta_station
 WHERE id = :id
EOT;
    error_log($sql);
    $stmt = $db->prepare($sql);
...
// 데이터 등록 apikey 반환
if ($_POST["func"] == "add_station") {
    $id = isset($_POST["id"])?$_POST["id"]:'111';
    $name = isset($_POST["name"])?$_POST["name"]:'TEST';
    $address = isset($_POST["address"])?$_POST["address"]:'SEOUL';
    $latitude = isset($_POST["latitude"])?$_POST["latitude"]:37.2;
    $longitude = isset($_POST["longitude"])?$_POST["longitude"]:126.1;
    $url = isset($_POST["url"])?$_POST["url"]:'http://localhost/iot/sht_pdo.php';
    $graph = isset($_POST["graph"])?$_POST["graph"]:'http://localhost/iot/sht_img.php';
...
SQLSTATEMENT;
    ...
    $db->commit();
    $response = array("apikey" => $param[':apikey']);
...
EOT;
    ...
    $db->commit();
    die(json_encode("remove count:".$r));
}
